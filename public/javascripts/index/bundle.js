...
var socket = (0, _socket2.default)((0, _jquery2.default)('#main').attr('data-ipAddress') + '?' + socketQueryParameters); //サーバーのIPアドレスに対して、WebSocket通信を開始するリクエストを送る
...
socket.on('start data', function (startObj) {
  //start dataとい名前のデータを受け取った時に実行される
...
socket.on('map data', function (compressed) {
  // map dataという名前のデータを受け取った時に実行される
...
function calculationBetweenTwoPoints(pX, pY, oX, oY, gameWidth, gameHeight, raderCanvasWidth, raderCanvasHeight) {
  // Playerの(pX,pY)と、オブジェクトの(oX,oY)としている。
...
(0, _jquery2.default)(window).keydown(function (event) {
  ...
  switch (event.key) {
    ...
    case ' ':
      ...
      var missileId = Math.floor(Math.random() * 10000) + ',' + gameObj.myPlayerObj.socketId + ',' + gameObj.myPlayerObj.x + ',' + gameObj.myPlayerObj.y;
