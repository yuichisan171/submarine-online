...
const socket = io($('#main').attr('data-ipAddress') + '?' + socketQueryParameters); //サーバーのIPアドレスに対して、WebSocket通信を開始するリクエストを送る
...
function ticker() {
  ...
  gameObj.ctxScore.clearRect(0, 0, gameObj.scoreCanvasWidth, gameObj.scoreCanvasHeight); // scoreCanvasもまっさら
...
function drawRadar(ctxRader) { // 潜水艦のレーダーを描画する関数
  const x = gameObj.raderCanvasWidth / 2;
  const y = gameObj.raderCanvasHeight / 2;
  const r = gameObj.raderCanvasWidth * 1.5 / 2; // 対角線の長さの半分

  ctxRader.save(); // セーブ。現在のキャンバスの状態を保存する。

  ctxRader.beginPath(); // 新しい描画を始める
  ctxRader.translate(x, y);
  ctxRader.rotate(getRadian(gameObj.deg)); // 座標をgameObj.deg度回転させる

  ctxRader.fillStyle = 'rgba(0, 220, 0, 0.5)'; // 半透明の緑色

  ctxRader.arc(0, 0, r, getRadian(0), getRadian(-30), true);
  ctxRader.lineTo(0, 0); //円弧を書いた筆で中心に向かって線を引く

  ctxRader.fill(); //描画したエリアを塗りつぶす

  ctxRader.restore(); // 元の設定を取得
  gameObj.deg = (gameObj.deg + 5) % 360;
}

function drawSubmarine(ctxRader, myPlayerObj) { // 潜水艦の画像を表示する関数
...
function drawMissiles(ctxScore, missilesMany) { // ミサイルの所持数
  for (let i = 0; i < missilesMany; i++) {
    ctxScore.drawImage(gameObj.missileImage, 50 * i, 80);
  }
}

function drawAirTimer(ctxScore, airTime) { // 酸素の残量表示
...
socket.on('start data', (startObj) => { //start dataとい名前のデータを受け取った時に実行される
...
socket.on('map data', (compressed) => { // map dataという名前のデータを受け取った時に実行される
...
function getRadian(kakudo) { // 角度をラジアンに変換する関数
  return kakudo * Math.PI / 180 // 角度 * π / 180という計算式でラジアンに変換する
}

function drawMap(gameObj) { // マップを描画する関数
  // 敵プレイヤーとNPCの描画
  ...
  for (let [index, item] of gameObj.itemsMap) {
    ...
    if (distanceObj.distanceX <= (gameObj.raderCanvasWidth / 2) && distanceObj.distanceY <= (gameObj.raderCanvasHeight / 2)) { //自分から半径250pxの範囲にあるものだけを描画
      const degreeDiff = calcDegreeDiffFromRadar(gameObj.deg, distanceObj.degree);
      const toumeido = calcOpacity(degreeDiff);
  ...
  for (const [airKey, airObj] of gameObj.airMap) {
    ...
    if (distanceObj.distanceX <= (gameObj.raderCanvasWidth / 2) && distanceObj.distanceY <= (gameObj.raderCanvasHeight / 2)) { // 同上
      const degreeDiff = calcDegreeDiffFromRadar(gameObj.deg, distanceObj.degree);
      const toumeido = calcOpacity(degreeDiff);
...
function calculationBetweenTwoPoints(pX, pY, oX, oY, gameWidth, gameHeight, raderCanvasWidth, raderCanvasHeight) { // Playerの(pX,pY)と、オブジェクトの(oX,oY)としている。
...
function calcTwoPointsDegree(x1, y1, x2, y2) { // 二点間の角度を求める関数
  const radian = Math.atan2(y2 - y1, x2 - x1);
  const degree = radian * 180 / Math.PI + 180;
  return degree;
}

function calcDegreeDiffFromRadar(degRader, degItem) { // アイテムとレーダーとの角度の差を計算する関数
...
function calcOpacity(degreeDiff) {
  const deleteDeg = 270;
  degreeDiff = degreeDiff > deleteDeg ? deleteDeg : degreeDiff; // もう少しだけ暗くする
...
$(window).keydown(function (event) {
  ...
  switch (event.key) {
    ...
    case ' ': // スペースキー
      ...
      const missileId = Math.floor(Math.random() * 10000) + ',' + gameObj.myPlayerObj.socketId + ',' + gameObj.myPlayerObj.x + ',' + gameObj.myPlayerObj.y;
...
function sendMissileEmit(socket, direction) { // ミサイルが発車したことをサーバーに送信する関数
  socket.emit('missile emit', direction);
}

function moveInClient(myPlayerObj, flyingMissilesMap) { // ミサイルと自分自身を移動させる関数
  if (myPlayerObj.isAlive === false) {
